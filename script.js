...
const entries = {
    ar: "1",
    apiKey: "6Lchf8UaAAAAAPz-Wj3dWTwpA8gzN_XBLa97zgNL",
    co: "aHR0cHM6Ly93d3cuYnVyZ2Vya2luZy5lczo0NDM",
    hl: "es",
    v: "xds0rzGrktR88uEZ2JUvdgOY",
    size: "invisible",
    cb: "dbkdfe2xcx6k"
}

const params = new URLSearchParams();
params.set("k", entries.apiKey)
params.set("ar", entries.ar)
params.set("co", entries.co)
params.set("hl", entries.hl)
params.set("v", entries.v)
params.set("size", entries.size)
params.set("cb", entries.cb)

const urls = {
    anchor: `https://www.google.com/recaptcha/api2/anchor?${params.toString()}`,
    reload: `https://www.google.com/recaptcha/api2/reload?k=${entries.apiKey}`
}

async function getData(url, user, pswd) {
    axios.get(url)
    .then(response => {
        token = parse(response.data).querySelector('input#recaptcha-token').getAttribute('value');

        axios.post(urls.reload, {
          v: params.get('v'),
          reason: 'q',
          c: token,
          k: entries.apiKey,
        ...
        .then(response => {
          const codedStr = btoa(`${btoa(user)}.${btoa(pswd)}`)
          const token = (JSON.parse(response.data.slice(5))[1])

          axios.post("https://apiusers.burgerking.es/login", {
            entercode: codedStr,
            token: `Bearer ${token}`
          }, {
            headers: {
              'Content-Type': 'application/json',
              'Origin': 'https://www.burgerking.es',
              'Accept': 'application/json',
            }
          })
          .catch(err => {
            console.log(`No existe ninguna cuenta asociada a ${user} o las credenciales son incorrectas.`)
          })
          .then(response => {
            if(response != undefined) {
              axios.get(`https://apiusers.burgerking.es/usersessionm?userId=${response.data.data.userid}&userS=${response.data.data.salesforceid}&avatar=null&rbicode=null`, {
                headers: {
                  'Content-Type': 'application/json',
                  'Origin': 'https://www.burgerking.es',
                  'Accept': 'application/json',
                  'Authorization': `Bearer ${response.data.data.usertoken}`
                }
              })
              .catch(err => {
                console.log("err2")
              ...
              .then(response => {
                console.log(`El usuario ${user} posee ${response.data.data.points} puntos`)
              })
            }
          })
        })
...
getData(urls.anchor, CORREO, CONTRASEÑA)
